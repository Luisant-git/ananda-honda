...
if (perms?.dashboard?.sales === true) setActiveTab('walkin'); // Walkin is related to Sales, making it first if sales access is there
  ...
  const fetchDashboardData = async (fDate, tDate) => {
    ...
    if (!activeFrom && !activeTo && (activeTab === 'sales' || activeTab === 'services' || activeTab === 'walkin')) {
      activeFrom = today;
      activeTo = today;
      setFromDate(today);
      setToDate(today);
    ...
    try {
      ...
      if (activeTab === 'sales') {
        ...
        const typeArg = serviceFilter === 'full' ? 'full payment' : (serviceFilter === 'advance' ? 'part payment' : undefined);
        data = await dashboardApi.getServicesDashboardStats(activeFrom, activeTo, typeArg);
  ...
  const handleReset = async () => {
    ...
    if (activeTab === 'sales' || activeTab === 'services' || activeTab === 'walkin') {
      resetFrom = today;
      resetTo = today;
    }
    ...
    try {
      ...
      if (activeTab === 'sales') {
        ...
        const typeArg = serviceFilter === 'full' ? 'full payment' : (serviceFilter === 'advance' ? 'part payment' : undefined);
        data = await dashboardApi.getServicesDashboardStats(resetFrom, resetTo, typeArg);
      } else if (activeTab === 'service_business') {
  ...
  const handleTabChange = (tab) => {
    ...
    setSearchTerm('');
    setExpandedMode(null);
    if (tab === 'sales' || tab === 'services' || tab === 'walkin') {
      setFromDate(today);
      setToDate(today);
    } else {
      setFromDate('');
  ...
  const downloadXML = () => {
    try {
      ...
      if (activeTab === 'services' && serviceFilter === 'full') type = 'Services Full Payment';
      if (activeTab === 'services' && serviceFilter === 'advance') type = 'Services Advance Payment';
  ...
  const renderSummaryCards = (data) => {
    ...
    const fullPaymentCount = data?.fullPaymentCount || 0;
    const advancePaymentCount = data?.advancePaymentCount || 0;
    ...
    const handleCardClick = () => {
      if (setCurrentView) {
        if (activeTab === 'sales') {
          setCurrentView('payment_collection');
        } else if (activeTab === 'services') {
          if (serviceFilter === 'full') {
            setCurrentView('service_payment_collection_full');
          } else if (serviceFilter === 'advance') {
            setCurrentView('service_payment_collection_advance');
          } else {
            setCurrentView('service_payment_collection');
          }
        }
      }
    };
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div 
          onClick={handleCardClick}
          className="cursor-pointer bg-brand-surface border border-brand-border/50 rounded-xl p-6 shadow-card hover:shadow-floating transition-all duration-300 relative overflow-hidden group flex flex-col"
        >
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
          ...
          <h3 className="text-3xl font-bold text-brand-text-primary mb-3">{totalCount || 0}</h3>
        </div>
        <div 
          onClick={handleCardClick}
          className="cursor-pointer bg-brand-surface border border-brand-border/50 rounded-xl p-6 shadow-card hover:shadow-floating transition-all duration-300 relative overflow-hidden group"
  ...
  const renderTable = (modes) => {
    ...
    let reportTitle = 'Services Payment Collection Report';
    if (activeTab === 'sales') reportTitle = 'Sales Payment Collection Report';
    else if (activeTab === 'services' && serviceFilter === 'full') reportTitle = 'Services Full Payment Report';
    else if (activeTab === 'services' && serviceFilter === 'advance') reportTitle = 'Services Advance Payment Report';
...
{(permissions?.dashboard?.sales || permissions?.dashboard?.service || permissions?.dashboard?.service_business) && (
  <div className="bg-brand-surface rounded-xl shadow-soft p-2 mb-6">
    <div className="flex flex-wrap gap-2">
      ...
      {/* Date Filter Section */}
      <div className="bg-brand-surface p-5 rounded-xl shadow-card border border-brand-border/50 flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8 relative overflow-hidden">
        ...
        {activeTab === 'services' && (
          <div className="flex bg-gray-100 rounded-lg p-1 w-full lg:w-auto mt-4 lg:mt-0">
            <button 
              onClick={() => setServiceFilter('all')} 
              className={`flex-1 lg:flex-none px-6 py-2 text-sm font-bold rounded-md transition-all ${serviceFilter === 'all' ? 'bg-white shadow text-brand-text-primary' : 'text-brand-text-secondary hover:text-brand-text-primary'}`}
            >
              All Types
            </button>
            <button 
              onClick={() => setServiceFilter('full')} 
              className={`flex-1 lg:flex-none px-6 py-2 text-sm font-bold rounded-md transition-all ${serviceFilter === 'full' ? 'bg-teal-500 shadow text-white' : 'text-brand-text-secondary hover:text-brand-text-primary'}`}
            >
              Full Payment
            </button>
            <button 
              onClick={() => setServiceFilter('advance')} 
              className={`flex-1 lg:flex-none px-6 py-2 text-sm font-bold rounded-md transition-all ${serviceFilter === 'advance' ? 'bg-cyan-500 shadow text-white' : 'text-brand-text-secondary hover:text-brand-text-primary'}`}
            >
              Advance
            </button>
          </div>
      ...
      {/* Walk-in Dashboard */}
      {activeTab === 'walkin' && <WalkinDashboard data={chartData} setCurrentView={setCurrentView} />}

      {/* Summary Cards */}
      {activeTab !== 'service_business' && activeTab !== 'walkin' && renderSummaryCards(chartData)}
